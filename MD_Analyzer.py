...
class MD_Analyzer(object):
    ...
    def create_lammps_input_anneal(self, Input_forcefield):
        ...
        ##### Anneal
        s.write('reset_timestep	0\n')
        s.write('fix MD3 all npt temp 300 5000 20 aniso 1.0 1.0 100.0\n')
        ...
        s.write('run 100000\n')
        s.write('unfix MD3\n')
        s.write('fix MD4 all npt temp 5000 300 20 aniso 1.0 1.0 100.0\n')
        s.write('run 500000\n')
        s.write('unfix MD4\n')
        ...
        s.write('dump DUMP4 all custom 5000 ' + 'anneal_' + self.LAMMPS_Data_file.replace('.data','') + self.simulation_ID + '.lammpstrj'+' id type x y z q #this size \n')
        s.write('thermo_style custom step etotal ke pe temp press pxx pyy pzz \n')
        s.write('thermo 1000\n')
